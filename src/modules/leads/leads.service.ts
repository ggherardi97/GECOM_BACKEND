...
export class LeadsService {
  ...
  async listLeads(user: AuthUser, query: ListLeadsQueryDto) {
    return this.repository.listLeads({
      tenantId: user.tenant_id,
      status: query.status as lead_status_enum | undefined,
      q: query.q,
  ...
  async getLeadById(user: AuthUser, leadId: string) {
    ...
    if (!this.canManageLead(user, lead) && !this.isPrivileged(user)) {
      // same effect, explicit for readability
  ...
  async createLead(user: AuthUser, dto: CreateLeadDto) {
    ...
    const lead = await this.repository.createLead({
      ...
      ownerUserId,
      status: (dto.status as lead_status_enum) ?? lead_status_enum.NEW,
      stageId,
  ...
  async updateLead(user: AuthUser, leadId: string, dto: UpdateLeadDto) {
    ...
    if (lead.status === lead_status_enum.CONVERTED) {
    ...
    const updated = await this.repository.updateLead({
      ...
      data: {
        ...
        ...(dto.status !== undefined ? { status: dto.status as lead_status_enum } : {}),
  ...
  async moveStage(user: AuthUser, leadId: string, dto: MoveLeadStageDto) {
    ...
    const status: lead_status_enum =
      stage.is_won
        ? lead_status_enum.CONVERTED
        : stage.is_lost
          ? lead_status_enum.DISQUALIFIED
          : lead.status === lead_status_enum.NEW
            ? lead_status_enum.WORKING
            : lead.status;
    ...
    const updated = await this.repository.updateLead({
      ...
      data: {
        ...
        status,
        ...(status === lead_status_enum.CONVERTED ? { converted_at: lead.converted_at ?? new Date() } : {}),
        ...(status !== lead_status_enum.CONVERTED ? { converted_at: null } : {}),
        updated_at: new Date(),
      },
    });
